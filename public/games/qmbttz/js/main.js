...
function() {
	...
	b = {
		shareDate: {
			...
			desc: "Challenge冰桶,并点名三位好友来Challenge.关怀渐冻人,支持弱势公益.",
			title: "冰桶Challenge",
			img_width: "120",
			img_height: "120"
		},
		...
		WeixinJSBridge && (window.weixinAPI = {
			...
			init: function() {
				...
				function() {
					...
					function() {
						_czc.push(["_trackEvent", "Share", "朋友圈"])
					})
				}),
				...
				function() {
					...
					function() {
						_czc.push(["_trackEvent", "Share", "好友"])
					})
				}),
				this
			}
...
function() {
	var a = {
		...
		initEvent: function() {
			if (!this.inited) {
				...
				function() {
					...
					b && c && d ? (Config.$el.share_tips.show(), (window.weixinAPI.showOptionMenu(), dataForWeixin.tTitle = "我Challenge" + a.score + "升冰水,【" + b + "," + c + "," + d + "】快来Challenge!关注ALS,传播公益.")) : alert("请点名3位好友")
				}),
				$("#d-over").on(b,
				function() {}),
				Config.$el.btn_retry.on(b,
				function() {
					_czc.push(["_trackEvent", "Click", "再来一次"]),
					a.reset()
				})
			}
		},
		...
		gameOver: function() {
			...
			dataForWeixin.tTitle = "我Challenge" + this.score + "升冰水,你也来Challenge吧!关注ALS,传播公益.";
			var a = 0,
			b = this;
...
function() {
	var a = {
		...
		initEvent: function() {
			...
			function() {
				_czc.push(["_trackEvent", "Click", "活动详情"]),
				Config.$el.pages.hide(),
				Config.$el.page_detail.show()
			}),
			Config.$el.btn_images.on(a,
			function() {
				_czc.push(["_trackEvent", "Click", "湿身照"]),
			...
			function() {
				_czc.push(["_trackEvent", "Click", "Challenge冰桶"]),
				Game.init()
			}),
