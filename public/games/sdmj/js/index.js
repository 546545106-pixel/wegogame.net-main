        ...
        if (c.status == 2014) {
            ...
            if (c.status == 2) {
                ...
                if (c.status == 2017) {
                    ...
                    toast("无法连接到网络，请检查网络Settings后再试")
                }
            }
        }
    }
...
Main.endGame = function (b) {
    ...
    // ih5game.setShare({
    //     desc: "我家母鸡下了" + Main.nowScore + "个蛋，超过了" + Main.getSharePercent(Main.nowScore) + "%的网友，给你当圣诞礼物也Yes极好的！"
    // });
    if (Main.nowScore < 40) {
        n += "\n不过弱爆了，全国的小伙伴都把\n你超越了！"
    } else {
...
Main.sendGameResult = function () {
    ...
    // ih5game.setShare({
    //     desc: "我家母鸡下了" + Main.nowScore + "个蛋，给你当圣诞礼物也Yes极好的！"
    // });
    c.error_count = 0;
    var b = null;
    if (gamef.platType == PLAT_BJY) {
