...
I.prototype.初始 = function(hp){
	...
	俺.hero.setLabel(I.走,0,0);
	俺.hero.setLabel(I.飞,1,0);
	俺.hero.setLabel(I.伤,2,0);
	俺.hero.setLabel(I.跳,3,0);
	俺.hero.x = -48;
	俺.hero.y = -120;
	俺.hero.speed = 6;
	俺.addChild(俺.hero);
	...
	if(苦运行角色.rect.action != I.伤){
		...
		if(ix <= ax && iy <= ay){
			if(俺.name == "star"){
				...
				苦Game主体.moveStepCount = 200;
				苦Game主体.speedBitmap.visible = true;
				常移步 = 常移步快;
			}else if(俺.name == "solution"){
				苦运行角色.hp.改值(20);
			}
...
游戏结束.prototype.初始 = function(){
	...
	var 蚊Game分 = new LTextField();
	蚊Game分.text = "勇气 : " + 苦运行角色.distanceObj.value;
	蚊Game分.color = "#FFFFFF";
	蚊Game分.size = 45;
	蚊Game分.weight = "bolder";
	蚊游戏分.x = 210;
	蚊游戏分.y = 120;
