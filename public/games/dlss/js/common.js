...
$(function(){
	...
	filesLoad();
	_smq.push(['custom', '杜杜足球赛', 'Game页面', 'StartLoad', ,1]);
	...
	$("#selecttype>.t2,#selecttype>.t3").bind(tapEvent,function(e){
		if($(this).hasClass("t2")){
			...
			_smq.push(['custom', '杜杜足球赛', 'Game页面', 'G点戳戳']);
			_gaq.push(['_trackEvent', '杜杜足球赛', 'Game页面', 'G点戳戳']);
		}else{
			gameType="shake";
			$(".start .speedup .txt").addClass("txt-y");
			_smq.push(['custom', '杜杜足球赛', 'Game页面', 'high抖抖']);
			_gaq.push(['_trackEvent', '杜杜足球赛', 'Game页面', 'high抖抖']);
...
function gend(){
	...
	c1.bind(tapEvent,function(e){
		if(coupons){
			...
			_smq.push(['custom', '杜杜足球赛', '电商', (gameType=="click"?'G点戳戳':'high抖抖')+'-Game Over-优惠券'+coupons.level]);
...
function grand(){
	...
	rb1.bind(tapEvent,function(e){
		...
		gameObj.gameReplay();
		rand.hide();
		_smq.push(['custom', '杜杜足球赛', 'Leaderboard', '再来一发']);
		_gaq.push(['_trackEvent', '杜杜足球赛', 'Leaderboard', '再来一发']);
	});
	function show(data){
		rand.show();
		showData(data);
